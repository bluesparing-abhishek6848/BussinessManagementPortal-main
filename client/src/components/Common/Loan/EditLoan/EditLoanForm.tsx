...
import type { IBranch, IBranchDropDown } from "../../Branch/BranchTypes";
...
const EditLoanForm = () => {
  ...
  const { data: userList } =
    useGet<GetResData<IUserDropDown>>(`users/drop-down?branch=${branch._id}`);
      ...
      <Grid container spacing={2}>
        ...
        <Grid size={{ xs: 12, sm: 6 }}>
          <Controller
            ...
            render={({ field }) => {
              const selectedUser =
                userList?.data.find((u) => u._id === field.value?._id) || null;
              return (
